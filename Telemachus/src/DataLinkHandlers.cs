...
namespace Telemachus
{
    ...
    public class FlyByWireDataLinkHandler : DataLinkHandler
    {
        ...
        {
            registerAPI(new ActionAPIEntry(
                dataSources => { lock (fbwLock) { yaw = checkFlightStateParameters(float.Parse(dataSources.args[0])); } return 0; },
                "v.setYaw", "Yaw [float yaw]", formatters.Default));

            registerAPI(new ActionAPIEntry(
                dataSources => { lock (fbwLock) { pitch = checkFlightStateParameters(float.Parse(dataSources.args[0])); } return 0; },
                "v.setPitch", "Pitch [float pitch]", formatters.Default));

            registerAPI(new ActionAPIEntry(
                dataSources => { lock (fbwLock) { roll = checkFlightStateParameters(float.Parse(dataSources.args[0])); } return 0; },
                "v.setRoll", "Roll [float roll]", formatters.Default));

            registerAPI(new ActionAPIEntry(
                dataSources => { lock (fbwLock) { on_attitude = int.Parse(dataSources.args[0]); } return 0; },
